...
public class WeatherActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if(weatherString != null){
            ...
            weatherLayout.setVisibility(View.INVISIBLE);//请求数据时先将ScrollView隐藏起来
            resquestWeather(mWeatherId);//从服务器请求天气数据
        }
    ...
    public void resquestWeather(final String weatherId){
        ...
        String weatherUrl = "http://guolin.tech/api/weather?cityid=" + weatherId + "&key=4f990d453b2846079cd6584df2c8bde0";
        HttpUtil.sendOkHttpRequest(weatherUrl, new Callback() {//向该地址发出请求
            @Override
            public void onResponse(Call call, Response response) throws IOException {
                ...
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (weather != null && "ok".equals(weather.status)){//如果服务器返回的status状态是ok，说明请求成功
                            //把返回的数据缓存到SharedPreferences中
                            SharedPreferences.Editor editor = PreferenceManager.getDefaultSharedPreferences(WeatherActivity.this).edit();
                            editor.putString("weather", responseText);
                            editor.apply();
                            mWeatherId = weather.basic.weatherId;
                            showWeatherInfo(weather);//把数据显示出来
                        }else {
